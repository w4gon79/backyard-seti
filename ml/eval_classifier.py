"""Evaluate trained classifier v2: metrics, plots, confusion matrix.

Usage:
    python ml/eval_classifier.py --target PROXCEN --crop-size 128 --snr-min 8
"""
...
def evaluate_classifier(checkpoint_path, target='PROXCEN', crop_size=128, snr_min=8, device='auto'):
    if device == 'auto':
    ...
    # Load crops + metadata
    crops, labels, metadata, hit_ids = load_clean_crops(target, config['crop_size'], snr_min)
    ...
    with torch.no_grad():
        for i in range(0, len(crops_dev), batch_size):
            batch_x = crops_dev[i:i+batch_size]
            batch_m = meta_dev[i:i+batch_size]
    ...
    try:
        ...
        if history:
            ...
            plt.suptitle('Classifier v2 Training History', fontsize=14)
            plt.tight_layout()
...
if __name__ == '__main__':
    ...
    evaluate_classifier(ckpt, target=args.target, crop_size=args.crop_size, snr_min=args.snr_min)
