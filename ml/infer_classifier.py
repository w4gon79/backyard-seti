"""Inference: classify all hits in DB using trained classifier.

Computes RFI probability for each hit. Writes scores to hits table.
Flags candidates (low RFI probability) and confidence levels.

Usage:
    python ml/infer_classifier.py --checkpoint checkpoints/classifier_best.pt
    python ml/infer_classifier.py --target PROXCEN --max-per-file 5000
"""
...
def add_classifier_columns(db_path):
    """Add classifier columns to hits table if missing."""
...
def classify_all_hits(checkpoint_path, target='PROXCEN', batch_size=512,
    ...
    """Classify all hits for a target using the trained classifier."""
    if db_path is None:
    ...
    c.execute("""
        SELECT h.id, h.source_file, h.freq, h.on_off, h.snr
        FROM hits h
    ...
    all_scores = {}  # hit_id -> rfi_probability
    total_scored = 0
    ...
    for file_idx, (source_file, file_hits) in enumerate(file_groups.items()):
        ...
        # Batch DB writes every 10 files
        if (file_idx + 1) % 10 == 0:
            _write_scores(c, all_scores)
            conn.commit()
            print(f"  --- Progress: {total_scored} scored, {len(all_scores)} pending writes ---")
    ...
    # Top candidates: ON hits with lowest RFI probability
    on_scores = {hid: s for hid, s in all_scores.items() 
                 if any(h['id'] == hid and h['on_off'] == 'ON' for h in hits)}
    
    if on_scores:
        sorted_candidates = sorted(on_scores.items(), key=lambda x: x[1])[:20]
        print(f"\nTop 20 ON candidates (lowest RFI probability):")
        for hit_id, score in sorted_candidates:
            row = c.execute(
                "SELECT freq, source_file, on_off, snr FROM hits WHERE id = ?", (hit_id,)
            ).fetchone()
            if row:
                print(f"  ID {hit_id}: {row['freq']:.6f} MHz SNR={row['snr']:.1f} "
                      f"rfi_prob={score:.4f} [{row['on_off']}] {row['source_file']}")
