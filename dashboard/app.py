    ...
    def run_scan():
        ...
        try:
            ...
            try:
                ...
                _invalidate_drift_caches(scan_id.split('_')[0])
                scan_state['log_lines'].append(
...
_drift_renders = {}   # job_id -> 'running' | 'done' | 'error'


def _invalidate_drift_caches(target):
    """Delete cached drift-scatter PNGs for a target after new scans import,
    so the Stack page regenerates them with the full hit population."""
    try:
        from db import get_db
        conn = get_db()
        rows = conn.execute('SELECT job_id FROM stack_jobs WHERE target = ?',
                            (target,)).fetchall()
        conn.close()
        for r in rows:
            p = os.path.join(STACK_OUTPUT_DIR, 'drift_%s.png' % r['job_id'])
            if os.path.isfile(p):
                try:
                    os.remove(p)
                except Exception:
...
def _render_drift_png(job_id, target, out_path):
    """Background worker: query every ON hit for target, rasterize scatter."""
    try:
        import numpy as np
        import matplotlib
        matplotlib.use('Agg')
        ...
        conn.close()
        if not freqs:
            _drift_renders[job_id] = 'error'
            return
        f = np.asarray(freqs, dtype=float)
        d = np.asarray(drifts, dtype=float)
        sv = np.log10(np.abs(np.asarray(snrs, dtype=float)) + 1.0)
        plt.rcParams.update({
        ...
        sc = ax.scatter(f, d, c=sv, s=0.4, cmap='plasma', alpha=0.6,
                        linewidths=0, rasterized=True)
        ax.set_xlabel('Frequency (MHz)')
        ax.set_ylabel('Drift rate (Hz/s)')
        ax.set_title('%s: every ON hit (%s total, all epochs), color = log10(SNR)'
                     % (target, '{:,}'.format(len(f))))
        ax.grid(alpha=0.4)
...
def api_stack_drift_scatter(job_id):
    ...
    Millions of points, rendered server-side in a background thread (first
    render takes minutes); returns 202 {'rendering': true} while working,
    cached PNG thereafter. ?refresh=1 forces a re-render."""
    out_path = os.path.join(STACK_OUTPUT_DIR, 'drift_%s.png' % job_id)
    ...
    if request.args.get('refresh') and os.path.isfile(out_path):
        try:
            os.remove(out_path)
        except Exception:
            pass

    if _drift_renders.get(job_id) == 'running':
        return jsonify({'rendering': True}), 202
    if os.path.isfile(out_path):
        return send_from_directory(os.path.dirname(out_path),
                                   os.path.basename(out_path))
    _drift_renders[job_id] = 'running'
    threading.Thread(target=_render_drift_png,
                     args=(job_id, target, out_path), daemon=True).start()
    return jsonify({'rendering': True}), 202
